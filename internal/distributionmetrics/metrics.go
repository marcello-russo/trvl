...
func RenderDashboard(cfg Config, githubSnapshots []GitHubSnapshot, npmSnapshots []NPMSnapshot) string {
	...
	fmt.Fprintf(&b, "# Distribution Metrics\n\n")
	fmt.Fprintf(&b, "Generated by `go run ./cmd/distribution-metrics`.\n\n")
	fmt.Fprintf(&b, "Privacy: uses only public aggregate GitHub release-asset and npm download APIs. No user IDs, IP addresses, referrers, or other PII are collected.\n\n")
	fmt.Fprintf(&b, "Launch baseline anchor: %s. Re-check targets: +30d %s, +90d %s.\n\n",
	...
	fmt.Fprintf(&b, "Baseline note: this snapshot is before the remaining directory listings are live; the official MCP Registry listing went live earlier on 2026-05-12.\n\n")

	if len(githubSnapshots) > 0 {
		latest := githubSnapshots[len(githubSnapshots)-1]
		fmt.Fprintf(&b, "## Latest Snapshot\n\n")
		fmt.Fprintf(&b, "- Captured: %s\n", latest.CapturedAt)
		fmt.Fprintf(&b, "- GitHub release asset downloads: %d\n", latest.TotalDownload)
		if len(npmSnapshots) > 0 {
			npm := npmSnapshots[len(npmSnapshots)-1]
			fmt.Fprintf(&b, "- npm `%s` downloads (%s to %s): %d", npm.Package, npm.RangeStart, npm.RangeEnd, npm.TotalDownload)
			if npm.Error != "" {
				fmt.Fprintf(&b, " (%s)", npm.Error)
			}
			fmt.Fprintf(&b, "\n")
		}
		fmt.Fprintf(&b, "\n")
	...
	fmt.Fprintf(&b, "## GitHub Release Downloads\n\n")
	fmt.Fprintf(&b, "| ISO week | Captured | Total downloads | Delta | Top release |\n")
	fmt.Fprintf(&b, "| --- | --- | ---: | ---: | --- |\n")
	prev := 0
	...
	fmt.Fprintf(&b, "\n## Files\n\n")
	fmt.Fprintf(&b, "- Weekly GitHub snapshots: `.internal/metrics/downloads-$YYYYWW.json`\n")
	fmt.Fprintf(&b, "- Weekly npm snapshots: `.internal/metrics/npm-$YYYYWW.json`\n")
	fmt.Fprintf(&b, "- This dashboard: `%s`\n\n", cfg.DashboardPath)
	fmt.Fprintf(&b, "Run weekly:\n\n")
